...
![蝶野雏蓝白小宇宙主题背景](themes/hina-chono-blue-white/background.png)

蓝白版从蝶野雏条纹上衣提取水手蓝、浅薰衣草蓝与冰白作为界面配色，并以独立主题 ID 提供，不会覆盖粉色版。

## 实机效果

下图来自真实 Codex Desktop 会话，展示了粉色主题、右侧面板适配和 Hina 宠物同时启用时的效果。蓝白版沿用相同布局：

![蝶野雏主题与 Hina 宠物实机效果](docs/screenshots/hina-theme-pet-live.png)
...
| Theme | `hina-chono-blue-white` | 蝶野雏 · 蓝白小宇宙 | `themes/hina-chono-blue-white/` |
| Pet | `hina` | Hina | `pets/hina/` |

每个主题目录内含一份 `theme.json` 和对应背景图：

- `themes/hina-chono/background.jpg`（3200 × 1801，粉色版）
- `themes/hina-chono-blue-white/background.png`（1672 × 941，蓝白版）
...
~/Library/Application Support/CodexDreamSkinStudio/themes/hina-chono-blue-white/
```

默认不会自动切换当前主题。立即应用原粉色主题可执行：
...
./tools/install-theme.sh hina-chono-blue-white --apply
```

## 打包 ZIP

生成只包含两套 Hina 主题、宠物、预览和安装脚本的分发包：
...
├── docs/screenshots/           # 真实 Codex Desktop 效果图
├── pets/hina/
│   ├── pet.json
│   └── spritesheet.webp
├── themes/
